...
#--------------------------------------------------------------------------------------#
    def detect_and_draw(imgmsg):
        ...
        circles = cv2.HoughCircles(img,cv.CV_HOUGH_GRADIENT,1,20,param1=50,param2=30,minRadius=10,maxRadius=20)
        if circles == None:
            return
        ...
        pixels.append(((MLx++int(np.round(dx3*2))) , (MLy+int(np.round(dy3*2)))))   
        length = len(pixels)
        print length
        sum_x = 0
        sum_y = 0
        ...
        cv2.setMouseCallback('detected circles',print_coords)
        cv2.waitKey(1)
