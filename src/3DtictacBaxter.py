...
#--------------------------------------------------------------------------------------------------------------------------------------------#
    ...
    def take_turn(self):
        ...
        for row in range(len(cube[0])):
            for space in range(len(cube[0][row])):
               # for level in range(len(cube)):
                copy = self.board.getCopy()
                if  not self.board.getPiece(len(cube)-1,row,space): # if space is free 
...
#--------------------------------------------------------------------------------------------------------------------------------------------#
    ...
    def range_callback(self,data):
        ...
        self.range = float(data.range / 1000)
            ...
            for row in range(3):
                for col in range(3):
                    if  self.board.getHighPiece(row,col) == self.playerPiece: # check each stack with a player piece on top
                        self.pose = self.board.getPosition(row,col)  
                        self.baxter_ik_move(self.limb, self.pose) 
                        rospy.sleep(1)
