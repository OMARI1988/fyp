...
#--------------------------------------------------------------------------------------------------------------------------------------------#
	...
	def take_turn(self):
		...
		def random_move(movesList):
			# Returns a valid move from the passed list on the passed board.
			# Returns None if there is no valid move.
			possibleMoves = []
			for i in movesList:
				if not self.board.getPiece(i[0],i[1]):
		...
		for row in spaces:
			for space in row:
				...
				if  not space.getContent(): # if space is free 
					copy.setPiece(spaces.index(row),row.index(space),self.robotPiece)
					if copy.checkWinner(self.robotPiece):
						space.setContent(self.robotPiece)	
						self.place_piece(spaces.index(row), row.index(space))
						return
		...
		for row in spaces:
			for space in row:
				copy = self.board.getCopy()
				if  not space.getContent(): # if space is free 
					copy.setPiece(spaces.index(row),row.index(space),self.playerPiece)
		...
		move = random_move([(0,0),(2,0),(0,2),(2,2)])
		if move != None:
			self.board.setPiece(move[0],move[1],self.robotPiece)
			self.place_piece(move[0], move[1])
			return      
				     
				     
		# Try to take the center, if it is free.
		if not self.board.getPiece(1,1):
			self.board.setPiece(1,1,self.robotPiece)
			self.place_piece(1,1)
			return 

		# Move on one of the sides.  
		move = random_move([(0,1), (1,0), (2,1), (1,2)])
		self.board.setPiece(move[0],move[1],self.robotPiece)
		self.place_piece(move[0], move[1])
		...
		for row in spaces:
			for space in row:
				if not space.getContent():
					dist = distance_to(pos,space.getPix()) 
					if dist < minDist:
						minDist = dist
						x = spaces.index(row)
						y = row.index(space)
...
#--------------------------------------------------------------------------------------------------------------------------------------------#
	...
	def place_piece(self, x, y, level=1):
		self.start_position()	
		self.find_object(1)				
		self.pickup_object()						
	...
	def locate_board(self):
		...
		for row in spaces:
			for space in row:
				space.setPix(self.pixelPoints[counter])
				counter += 1

		# store positions of board points in Board class model
		self.board.setPosition(1,1,centre)
		
		self.board.setPosition(2,1,(centre[0] + (self.boardHeight/3),
							centre[1],
							centre[2],
							centre[3],
							centre[4],
		...
		self.board.setPosition(0,1,(centre[0] - (self.boardHeight/3),
							centre[1],
							centre[2],
							centre[3],
							centre[4],
		...
		self.board.setPosition(1,2,(centre[0],
		...
		self.board.setPosition(1,0,(centre[0],
							centre[1] - (self.boardHeight/3),
							centre[2],
							centre[3],
							centre[4],
